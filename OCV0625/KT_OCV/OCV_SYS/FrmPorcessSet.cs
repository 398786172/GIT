    ...
    public partial class FormPorcessSet : Form
    {
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            if (cmbOCVType.Text == "OCV2"|| cmbOCVType.Text == "OCV3")
            {
                ...
                #region 正极 
                if (double.TryParse(txtDownLmt_SVP.Text, out Val1) == false)
                {
                    MessageBox.Show("壳压/正极电压下限输入出错");
                    return;
                }
                else if (double.TryParse(txtUpLmt_SVP.Text, out Val2) == false)
                {
                    MessageBox.Show("壳压/正极电压上限输入出错");
                    return;
                }

                if (Val2 > Val1)
                {
                    INIAPI.INIWriteValue(BattTypePath, strConfigName, "DownLmt_SVP", Val1.ToString());
                    INIAPI.INIWriteValue(BattTypePath, strConfigName, "UpLmt_SVP", Val2.ToString());
                }
                else
                {
                    MessageBox.Show("错误;设置壳压/正极电压值上限小于下限");
                    return;
                }
                #endregion  正极
        ...
        private void cmb_Config_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cmbOCVType.Text == "OCV2" || cmbOCVType.Text == "OCV3")
            {
                ...
                txtUpLmt_SVP.Text = INIAPI.INIGetStringValue(BattTypePath, strConfigName, "UpLmt_SVP", null);
                txtDownLmt_SVP.Text = INIAPI.INIGetStringValue(BattTypePath, strConfigName, "DownLmt_SVP", null);
