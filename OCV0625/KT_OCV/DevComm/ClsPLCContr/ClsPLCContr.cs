...
using OCV.DevComm.ClsPLCContr.ClsPLCAddrCollection;
...
namespace OCV
{
    public class ClsPLCContr
    {
        ...
        private OperateResult connectResult = null;//连接结果
                ...
                try
                {
                    if (connectResult.IsSuccess)
                    {
                        ...
                        if (opr.IsSuccess)
                        {
                            ...
                            // ClsPLCValue.PlcValue.Plc_ScanFinshReply = opr.Content[1];//复位
                            ClsPLCValue.PlcValue.Plc_RequestTest = opr.Content[3];
                            ClsPLCValue.PlcValue.Plc_TestFinshReply = opr.Content[4];//PLC_测定结束应答
                            //ClsPLCValue.PlcValue.Plc_ResetReply = opr.Content[5];
                            ClsPLCValue.PlcValue.Plc_InitReply = opr.Content[7];
                            ...
                            ClsPLCValue.PlcValue.Plc_IO_ZeroCompletion = opr.Content[24]; //PLC_初始化完成
                        }
                        else
                        { 
                            //读取异常 
