...
namespace OCV
{
    ...
    public class ClsProcess
    {
        ...
        public void InitPlc()
        {
            ...
            ClsPLCValue.PlcValue.Plc_InitReply = 0;//PLC初始化回应信号
            Action act = delegate
            {
                ...
                do
                {
                    ...
                    if (ClsPLCValue.PlcValue.Plc_InitReply == 1)//PLC初始化完成
                    {
        ...
        private void ThreadProcess()
        {
            ...
            while (true)
            {
                ...
                {
                    mPLCContr.WriteDB(mPLCContr.mPlcAddr.PC_指示上位机有报警, 8);//val=1
                    ClsGlobal.mPLCContr.WriteDB("W11", (ushort)1);//程序异常停止反馈PLC
            ...
            switch (mStep)
            {
                case 1: //检查是否有托盘进入
                    if (ClsPLCValue.PlcValue.Plc_HaveTray == 1)
                    {
                        ...
                        mInfoSend("托盘已进入.");
                        ClsGlobal.OCVTestContr.InitPara();//DataGridView表数据清空
                ...
                case 4:
                    try
                    {
                        ...
                        if (temp == "NG" || temp == "ERROR")
                        {
                            ...
                            mPLCContr.WriteDB(mPLCContr.mPlcAddr.PC_指示上位机有报警, 2);//val=1
                    ...
                    else if (ClsGlobal.OCV_RunMode == eRunMode.NormalTest)
                    {
                        if (isTestAgainState == false)
                        {
                            ...
                            if (ClsGlobal.IsLocalOCVType == 0 && ClsGlobal.IsAutoMode == true)//远程
                            {
                                ...
                                //if (currType != 2 && currType != 3)//OCV获取工序不符合，报警处理
                                //{
        ...
        private void TestTemp()
        {
            try
            {
                ...
                for (int i = 0; i < ClsGlobal.TrayType; i++)
                {
                    ...
                    ClsGlobal.listETCELL[i].PostiveTMP = ClsGlobal.G_dbl_P_TempArr[i];//0
