...
namespace OCV
{
    public partial class FrmSystemSetting : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            //通道测试异常计数复测设置
            INIAPI.INIWriteValue(ClsGlobal.mSettingPath, "System", "SetEnOCV  ", txtOCVTest.Text);
        ...
        private void FrmSystemSetting_Load(object sender, EventArgs e)
        {
            ...
            Val5 = INIAPI.INIGetStringValue(ClsGlobal.mSettingPath, "System", "SetEnOCV ", null);
            txtOCVTest.Text = Val5;
            //设备序号
            ...
            cmbTestCount.Text = INIAPI.INIGetStringValue(ClsGlobal.mSettingPath, "System", "ProcTestCount", null);
            loadFalg = 1;
        }
        ...
        private void btnModify_Click(object sender, EventArgs e)
        {
            try
            {
                ClsWSDLHandle.MakeOCVWSDLReference(ClsGlobal.MESURL_bindtray, "NG.WSBindTrayReference", "WSReference1.cs");
                ClsWSDLHandle.MakeOCVWSDLReference(ClsGlobal.MESURL_OCV, "NG.WSOCVReference", "WSReference2.cs");

                Thread.Sleep(3000);

                ClsWSDLHandle.MakeDLL("WSReference1.cs", "WSBindTray.dll");
                ClsWSDLHandle.MakeDLL("WSReference2.cs", "WSOCV.dll");

                Thread ThreadDeleteFile = new Thread(DeleteFile);
        ...
        private void DeleteFile()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    FileInfo file = new FileInfo(Application.StartupPath + @"\TempFiles\WSReference1.cs");
                    FileInfo file2 = new FileInfo(Application.StartupPath + @"\TempFiles\WSReference2.cs");
                    if (file.Exists == true)
                    {
                        file.Delete();
                    }
        ...
        private void btnset_Click(object sender, EventArgs e)
        {
            ...
            int Val1;
            frmUserPwd frmPwd = new frmUserPwd(PwdType.PROCESS, "工艺返工次数"+ cmbTestCount.Text);
            if (frmPwd.ShowDialog() != DialogResult.OK)
            {
                return;
            }
              
            if (int.TryParse(cmbTestCount.Text, out Val1) == false)
            {
                MessageBox.Show("工艺返工次数设置错误");
                return;
            }
            INIAPI.INIWriteValue(ClsGlobal.mSettingPath, "System", "ProcTestCount", cmbTestCount.Text);
            ClsGlobal.ProcTestCount = Val1;
            MessageBox.Show("工艺返工次数保存成功!", "提示");
        ...
        private void cmbTestCount_SelectedIndexChanged(object sender, EventArgs e)
        {

        }

        private void tab位移传感器_Click(object sender, EventArgs e)
        {
