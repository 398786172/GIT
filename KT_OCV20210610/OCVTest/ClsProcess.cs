...
namespace OCV
{
    ...
    public class ClsProcess
    {
            ...
            try
            {
                switch (mStep)
                {
                    ...
                    case 10:
                        ...
                        if (ExportToLocFinish == true && ExportToServerFinish == true && ErrUpLoadngData == true && ExportToMesFinish == true)
                        {
        ...
        {
            string addr = CSVPath + "\\OCV" + ClsGlobal.OCVType.ToString() + "_" + TrayCode + "_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".csv";
            ...
            SWR.WriteLine("通道号, 托盘条码,电池条码,总判定结果,开路电压(mV),ACIR(mΩ),测试结果,NG原因,壳体电压(mV),壳体电压检测结果,壳体电压NG原因,正极温度(℃),负极温度(℃),压降△V,压降△V极差,压降△V极差判定结果,压降△V极差NG原因,ACIR极差,ACIR极差判定结果,ACIR极差NG原因,补偿前电压(mV),补偿后电压(mV),容量,测试时间,工站");
            for (int i = 0; i < ClsGlobal.listETCELL.Count; i++)
            {
