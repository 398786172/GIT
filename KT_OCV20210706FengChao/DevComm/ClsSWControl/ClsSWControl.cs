...
namespace OCV
{
    ...
    public class ClsSWControl
    {
        ...
        /// 切换测量电压
        /// </summary>
        /// <param name="POSConnType">正极连接类型:  正极对负极->1, 壳体对负极->2,壳体对正极->3 </param>
        ...
        /// <summary>
        /// 同步切换测量命令: 切换1区的通道, 同时切换2区的对应通道 
        /// 例如, 总通道数60通道, 1区分配32通道,2区分配28通道; 按顺序通道号从1区开始数起, 则1区通道号: 1~32,2区通道号:33~60.
        /// 当执行该命令切换第2通道时, 将同时切换第34通道.
        /// </summary>
        /// <param name="TestType">测量类型:   (1区测正负极电压, 2区测内阻)->1, (1区测内阻, 2区测正负极电压)->2
        /// </param>
        /// <param name="Channel">切换的1区通道号. 通道范围约束为 MIN(1区通道数, 2区通道数) 
        /// 例如, 范围:28= MIN(32, 28), 注意1区的29~32通道不进行同步测量, 因为2区没有对应
        /// </param>
        public void ChannelSYNVoltIRSwitchContr(int TestType, int Channel)
        {
            byte[] sendBuf = new byte[13];
            int ErrId = 0;
