...
namespace OCV
{
    ...
    public class ClsOCVContr
    {
        ...
        public ClsSWControl SwitchDev;          //切换控制
        public ClsDMM_Ag344X mDmm;              //万用表
        public ClsHIOKI4560 mRTester_BT4;       //内阻仪BT4560控制
       ...
       // int mStartBattNum;                                                //开始测量的电池通道号,随当前测试位而变动                
        int CHCount = ClsGlobal.TrayType;                             //测试通道数 
        ...
        public ClsOCVContr(SerialPort SW_SP, SerialPort DMM_OCV, FrmSys f1)
        {
            try
            {
                //没万用表时, 暂时屏蔽------------
                ...
                SwitchDev = new ClsSWControl(SW_SP, 1, ClsGlobal.SwitchVersion);
        ...
        public ClsOCVContr(SerialPort SW_SP, string DMM_OCV, FrmSys f1)
        {
            try
            {
                //没万用表时, 暂时屏蔽------------
        ...
        /// 
        /// </summary>
        /// <param name="SW_SP">通道切换板</param>
        /// <param name="MMT_SP">万用表</param>
        /// <param name="RTester_SP">内阻仪端口</param>
        /// <param name="f1"></param>
        public ClsOCVContr(SerialPort SW_SP, SerialPort DMM_OCV, string RTester_SP, FrmSys f1)
        {
            try
            {
                mDmm = new ClsDMM_Ag344X(DMM_OCV);
                mDmm.InitControl_IMM();
                mRTester_BT4 = new ClsHIOKI4560(RTester_SP);
                mRTester_BT4.mRang = ClsGlobal.BT4560RANG;
                mRTester_BT4.Freq = ClsGlobal.TestFreq;
                mRTester_BT4.InitRTester = ClsGlobal.InitBT4560;
                mRTester_BT4.V_SampleRate = 3;
                mRTester_BT4.Z_SampleRate = 2;   //中速
                mRTester_BT4.InitControl_IMM();
        ...
        /// <param name="RTester_SP">内阻仪端口</param>
        /// <param name="f1"></param>
        public ClsOCVContr(SerialPort SW_SP, string DMM_OCV, string RTester_SP, FrmSys f1)
        {
            try
            {
                mDmm = new ClsDMM_Ag344X(DMM_OCV);
                ...
                SwitchDev = new ClsSWControl(SW_SP, 1 , 2);
        ...
        private void TestVoltForProc()
        {
            ...
            try
            {
                ...
                {
                    //Thread.Sleep(150);
                    //mDmm.InitControl_EXT((uint)CHCount);
                    mDmm.InitControl_IMM();
                    Thread.Sleep(30);
                }
                catch (Exception ex)
                ...
                for (int Num = 0; Num < ClsGlobal.TrayType; Num++)
                {
                    ...
                    if (theIRSample * 1000 > 100)
                    {
                        ManualAdjust.Invoke(new EventHandler(delegate
                        {
                            ...
                            ManualAdjust.arrTxt_IRSampleShow[Num].BackColor = Color.Red;
                            ManualAdjust.arrTxt_IRSampleShow[Num].Text = "失败";
                        }));
                        continue;
                        //break;
                    }
        ...
        //            switch (ClsGlobal.mShellTestType)   //不测壳体:0   需要测壳体:1
        //            {
        //                case 1:
        ...
        //                        SwitchDev.ChannelVoltSwitchContr(1, ActualNum);        //正极对负极
        ...
        //                    SwitchDev.ChannelVoltSwitchContr(1, 0);      //结束,通道全部关断
        //                    Thread.Sleep(150);
        ...
        //                        //壳体电压异常判断
        //                        if (theDMMVolt * 1000 < ClsGlobal.ShellVolDownLmt)
        //                        {
        //                            ClsGlobal.arrShellVolErrChannel[Num] = 1;
        //                            ClsGlobal.ShellVolIsErr = true;
        //                        }
        //                        else if (theDMMVolt * 1000 > ClsGlobal.ShellVolUpLmt)
        //                        {
        //                            ClsGlobal.arrShellVolErrChannel[Num] = 2;
        //                            ClsGlobal.ShellVolIsErr = true;
        //                        }
        //                        else
        //                        {
        //                            ClsGlobal.arrShellVolErrChannel[Num] = 0;
        //                        }
        //                    }
        //                    SwitchDev.ChannelVoltSwitchContr(2, 0);      //结束,通道全部关断
        //                    Thread.Sleep(150);
        ...
        //                    {
        //                        //电压数据
        //                        if (Math.Abs(arrDMMVolt[i]) < 1e+6)
        //                        {
        //                            ClsGlobal.G_dbl_VoltArr[i] = arrDMMVolt[i] * 1000 + double.Parse(ClsGlobal.ArrVoltAdjustPara[i]);
        //                            ClsGlobal.G_dbl_VshellArr[i] = arrShellVolt[i] * 1000 + double.Parse(ClsGlobal.ArrVoltAdjustPara[i]);
        //                        }
        //                        else
        //                        {
        //                            ClsGlobal.G_dbl_VoltArr[i] = 99999;
        ...
        //                        {
        //                            //内阻数据
        //                            ClsGlobal.G_dbl_AcirArr[i] = arrACIR_BT4[i];
        //                        }
        //                        else
        //                        {
        //                            ClsGlobal.G_dbl_AcirArr[i] = 99999;
        ...
        //                        ClsGlobal.listETCELL[i].OCV_V1 = ClsGlobal.G_dbl_VoltArr[i];
        //                        ClsGlobal.listETCELL[i].OCV_V2 = ClsGlobal.G_dbl_VshellArr[i];
        //                        ClsGlobal.listETCELL[i].Acir = ClsGlobal.G_dbl_AcirArr[i];

        //                        ClsGlobal.listETCELL[i].OCV_Write_Time = ClsGlobal.TestStartTime.ToString(); //ClsGlobal.G_dbl_TimeArr[i];
        //                        ClsGlobal.listETCELL[i].CODE = CheckVoltValNG(ClsGlobal.G_dbl_VoltArr[i], ClsGlobal.G_dbl_AcirArr[i]);
        ...
        //                                if (i < Val)
        //                                {
        //                                    mForm.dgvTest.Rows[i].Cells["Col_OCV"].Value = ClsGlobal.G_dbl_VoltArr[i].ToString("F3");
        //                                    mForm.dgvTest.Rows[i].Cells["Col_CaseOCV"].Value = ClsGlobal.G_dbl_VshellArr[i].ToString("F3");
        //                                    mForm.dgvTest.Rows[i].Cells["Col_ACIR"].Value = ClsGlobal.G_dbl_AcirArr[i].ToString("F3");
        //                                    mForm.dgvTest.Rows[i].Cells["Col_CODE"].Value = CheckVoltValNG(ClsGlobal.G_dbl_VoltArr[i], ClsGlobal.G_dbl_AcirArr[i]);

        //                                    if (ClsGlobal.En_CheckShellVol == true && ClsGlobal.arrShellVolErrChannel[i] != 0)
        //                                    {
        //                                        mForm.dgvTest.Rows[i].Cells["Col_CaseOCV"].Style.BackColor = Color.Red;
        //                                    }
        //                                }
        //                                else
        //                                {
        //                                    mForm.dgvTest.Rows[i - Val].Cells["Col_OCV2"].Value = ClsGlobal.G_dbl_VoltArr[i].ToString("F3");
        //                                    mForm.dgvTest.Rows[i - Val].Cells["Col_CaseOCV2"].Value = ClsGlobal.G_dbl_VshellArr[i].ToString("F3");
        //                                    mForm.dgvTest.Rows[i - Val].Cells["Col_ACIR2"].Value = ClsGlobal.G_dbl_AcirArr[i].ToString("F3");
        //                                    mForm.dgvTest.Rows[i - Val].Cells["Col_CODE2"].Value = CheckVoltValNG(ClsGlobal.G_dbl_VoltArr[i], ClsGlobal.G_dbl_AcirArr[i]);

        //                                    if (ClsGlobal.En_CheckShellVol == true && ClsGlobal.arrShellVolErrChannel[i] != 0)
        //                                    {
        //                                        mForm.dgvTest.Rows[i - Val].Cells["Col_CaseOCV2"].Style.BackColor = Color.Red;
        //                                    }
        //                                }
        //                            }

        //                            for (int i = 0; i < ClsGlobal.TrayType / 2; i++)
        //                            {
        //                                mForm.dgvTest.Rows[i].Cells["Col_Num"].Value = (i + 1).ToString();
        //                                mForm.dgvTest.Rows[i].Cells["Col_Num2"].Value = (i + 1 + ClsGlobal.TrayType / 2).ToString();
        //                            }
        //                        }
        //                        ));
        //                    }
