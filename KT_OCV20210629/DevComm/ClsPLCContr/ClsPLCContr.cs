...
namespace OCV
{
    public class ClsPLCContr
    {
        ...
        public void DrvInitial(int Port, string Addr)
        {
            ...
            //PlcTcpNet.SA1 = 192;
            //PlcTcpNet.DA2 = 0;
            PlcTcpNet.ByteTransform.DataFormat = //HslCommunication.Core.
                DataFormat.CDAB;//DataFormat.CDAB;
            ...
            while (true)
            {
                try
                {
                    if (connectResult.IsSuccess)
                    {
                        ...
                        ushort arrLength = 34; //30;      //数组长度 30 //
                        //HslCommunication.
                        ...
                        if (opr.IsSuccess)
                        {
                            ...
                            ClsPLCValue.PlcValue.Plc_Period = opr.Content[21];    //PLC单次运动时间
                            ClsPLCValue.PlcValue.Plc_IO_ZeroCompletion = opr.Content[24];//X轴回零完成
                        ...
                        else
                        {
                            ...
                            {
                                connectResult = new OperateResult() { IsSuccess = false };// HslCommunication.
                            }
                        }
                        register = "W160";
                        arrLength = 2;
                        ...
                        Uopr = PlcTcpNet.ReadUInt16("W140", 2);
                        if (opr.IsSuccess)
                        {
                            //IO
                            ...
                            ClsPLCValue.PlcValue.Plc_IO_BlockCylUp = this.CheckBit16(Uopr.Content[0], 8);
                            ClsPLCValue.PlcValue.Plc_IO_BlockCylDown = this.CheckBit16(Uopr.Content[0], 9);
                    ...
                    else
                    {
                        try
                        {
                            ...
                            if (!connectResult.IsSuccess) //若连接失败
                            {
                                ...
                                ClsPLCValue.PlcValue.Plc_IO_ProbeCylClose = -1;        //探针左压合伸
                                ClsPLCValue.PlcValue.Plc_IO_ProbeCylOpen = -1;         //探针左压合缩
                                ClsPLCValue.PlcValue.Plc_IO_ProbeCylClose1 = -1;        //探针右压合伸
                                ClsPLCValue.PlcValue.Plc_IO_ProbeCylOpen1 = -1;         //探针右压合缩
                                ...
                                ClsPLCValue.PlcValue.Plc_IO_YanwuUp = -1;              //烟雾传感器前
                                ClsPLCValue.PlcValue.Plc_IO_YanwuDown = -1;            //烟雾传感器后
                                ClsPLCValue.PlcValue.Plc_IO_Opendoor = -1;             //门开关
                                ClsPLCValue.PlcValue.Plc_IO_Stop = -1;                  //急停
                                ClsPLCValue.PlcValue.Plc_IO_Alaguntong = -1;            //滚筒报警
