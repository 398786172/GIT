...
namespace OCV
{
    ...
    public class ClsHIOKI365X
    {
        ...
        /// <summary>
        /// 内阻仪初始化(内部触发方式) 量程300毫欧
        /// </summary>
        /// <param name="SampleRateType">采样速率: SLOW->1 MED->2, FAST->3</param>
        /// <param name="type"> 测试内阻和电压 ->1 , 只测试内阻,不测电压 -> 2</param>
        public void InitControl_IMM(int SampleRateType, int type)
        {
            ...
            if (type == 1)
            {
                SPort.WriteLine(":FUNC RV");        //测试内阻和电压 
            }
            else if (type == 2)
            {
        ...
        /// 量程:300毫欧
        /// </summary>
        public void InitControl_IMM_BigRange(int SampleRateType, int type)
        {
            InitControl_IMM(SampleRateType, type);    //此设备只用量程:300毫欧  20180402
        }
        ...
        public void ReadDataRV(out double VoltVal, out double IMPVal)
        {
            try
            {
                string strVal;
                ...
                try
                {
                    volt = double.Parse(arrStrVal[1]?.Replace(" ", ""));
                }
                catch (Exception ex)
                {
                ...
                VoltVal = volt;
                double tmp1;
                if (!string.IsNullOrEmpty(arrStrVal[0]))
                {
                    if (double.TryParse(arrStrVal[0]?.Replace(" ", ""), out tmp1) == true)
                    {
                        IMPVal = tmp1;
                    }
                    else
