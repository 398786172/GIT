...
namespace OCV
{
    public partial class FormSysSet : Form
    {
        ...
        void FormSysSet_Load(object sender, System.EventArgs e)
        {
            ...
            {
                radioBtnSocket.Checked = true;
                IPtxt.Enabled = true;
                Porttxt.Enabled = true;
                cmbScan_Port.Enabled = false;
            }
            else if (ClsGlobal.CodeScanMode == 2)
            {
                radioButton2.Checked = true;
                cmbScan_Port.Enabled = true;
                IPtxt.Enabled = false;
            ...
            //IP端口设置
            IPtxt.Text = ClsGlobal.CodeScanIP;
            Porttxt.Text = ClsGlobal.CodeScanPort;
        ...
        private void button18_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("设备号输入错误", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            {
                reader = ClsGlobal.sql.UpdateValues("System", new string[] { "Value" }, new string[] { "2" }, "Parameter", "Scan_Setting_Mode", "=");
            ...
            if (radioBtnSocket.Checked)
            {
                reader = ClsGlobal.sql.UpdateValues("System", new string[] { "Value" }, new string[] { "1" }, "Parameter", "Scan_Setting_Mode", "=");
                if (IPtxt.Text != "" && Porttxt.Text != "")
                {
                    SQLiteDataReader reader1 = null, reader2 = null;
                    reader1 = ClsGlobal.sql.UpdateValues("System", new string[] { "Value" }, new string[] { IPtxt.Text }, "Parameter", "Scan_IP", "=");
                    int count1 = reader1.RecordsAffected;
                    reader2 = ClsGlobal.sql.UpdateValues("System", new string[] { "Value" }, new string[] { Porttxt.Text }, "Parameter", "Scan_IP_Port", "=");
                    int count2 = reader2.RecordsAffected;
                    if (count1 > 0||count2>0)
                    {
                        ClsLogs.INIlogNet.WriteInfo("通信设置", "扫码器IP及端口设置:" + IPtxt.Text+"  "+Porttxt.Text + "成功");
                        ClsGlobal.SetParflag = true;
                    }
                    else
                    {
                        ClsLogs.INIlogNet.WriteFatal("通信设置", "扫码器IP及端口设置:" + IPtxt.Text + "  " + Porttxt.Text + "失败");
                        MessageBox.Show("扫码器IP端口设置异常", "异常", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                }
                else
        ...
        private void radioBtnSocket_CheckedChanged(object sender, EventArgs e)
        {
            cmbScan_Port.Enabled = false;
            IPtxt.Enabled = true;
            Porttxt.Enabled = true;
